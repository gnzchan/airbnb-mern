        ...
        return (
          <div className="mx-auto w-full max-w-lg">
            <p className="text-center">
              {`Logged in as ${userContext?.user?.firstName} ${userContext?.user?.lastName} (${userContext?.user?.email})`}
            </p>
      ...
      <div className="w-full flex justify-center mt-8">
        <nav className="grid grid-cols-3 gap-2">
          {routes.map((route) => (
            <Link
              key={route.href}
              className={`py-2 px-6 rounded-full flex gap-3 w-full justify-center ${
